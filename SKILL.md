...
- 带动效的前端交互层（CSS / JS / Canvas / Three.js 均可）
...
- 用户不用保持自己的电脑开机
- 页面不是纯静态排版，至少具备一套完整的入场 / 滚动 / 悬停 / 切页动效
...
- 首屏至少有一组入场动画，例如标题、说明、统计数字逐段 fade-up
- 导航在滚动后允许出现 blur / background / border 变化
- 卡片 hover 至少有一层轻量反馈，例如上浮、缩放、遮罩、标题变色
- 页面切换或 section 切换应有过渡，不要突兀闪切
- About / 联系方式 / 能力模块这类信息区，优先加 scroll reveal
- 如果用户希望更强视觉表现，可以增加 Canvas 或 Three.js 背景动画

实现边界：

- 优先用原生 CSS transition / keyframes / IntersectionObserver
- 需要持续运动背景时再用 `requestAnimationFrame`
- 如果用了 Three.js，必须确保移动端不会空白、卡死或遮挡主要内容
- 动画不能影响视频播放、筛选、弹窗、移动端滚动和首屏可读性
- 默认提供降级路径，脚本失败时文字和内容仍然可见
...
- hero 文案入场
- sticky filter / nav 的滚动状态过渡
- masonry 卡片 hover 动效
- 页面淡入淡出切换
- About 页的 stagger reveal
- 可选的 Three.js / Canvas 背景运动
